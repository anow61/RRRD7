...
async def _(event):
    ...
    if msg and msg.media:
        ...
        await event.edit("**تـم حفظ التـرحيب**. ")
    else:
...
@command(pattern="^.حذف")  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
    ...
    await event.edit(
        "**تـم حـذف الـترحـيب** "
        + "رسـالة الـتحريب كانـت `{}`.".format(cws.custom_welcome_message)
    )


@command(pattern="^.الترحيب")  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
    if hasattr(cws, "custom_welcome_message"):
        await event.edit(
            " **تم الـتطوير بـواسطه فـريق جـمثون** "
            + "**الـترحيب المضاف في هذه الـدردشه**\n\n`{}`.".format(cws.custom_welcome_message)
        )
    else:
        await event.edit("لـم يتم ايجـاد تـرحيب")
